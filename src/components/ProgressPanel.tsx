...
const STEPS: { key: string; label: string; stages: string[] }[] = [
  { key: "upload", label: "Upload", stages: ["created", "uploading", "uploaded"] },
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            <li key={step.key} className="flex items-center gap-3 text-sm">
              ...
              {active && <span className="text-xs text-gray-400">{job.message}</span>}
            </li>
          );
        })}
      </ol>
      {job.stage === "failed" && (
        <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">Erreur : {job.error}</p>
      )}
    </div>
  );
}
